...
const getforYou = async (req, res) => {
  ...
  try {
    ...
    if (result) {
      ...
      const degreeName = result.degree.substring(
        degree.length + 4,
        result.degree.length
      );
      res.json({
        progress: `${percentageCompleted.toFixed(2)}%`,
        level: degree,
        degree: degreeName,
      });
    } else res.status(404).json({ error: "No data found for this user." });
  } catch (error) {
    console.error(error);
