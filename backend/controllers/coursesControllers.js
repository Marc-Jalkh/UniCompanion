...
const getCourses = async (req, res) => {
  ...
  try {
    ...
    const courses = await db("users_courses as uc")
      ...
      .select("c.title", "co.crn", "co.location", "uc.grade")
...
async function getInstructorCourses(userId) {
  ...
  const semesterString =
    semester.semesterString + " " + (semester.year - 1) + "-" + semester.year;
  const filteredCourses = courses.filter(
    (course) => course.crn.slice(0, 5) == crn
  );
  ...
  for (let course of filteredCourses) {
    ...
    if (!coursesBySemester[semesterString]) {
